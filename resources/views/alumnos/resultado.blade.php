...
<div class="container">
    ...
    @php
        // Usar la evaluación centralizada si está disponible (fuente única de verdad)
        if (isset($evaluacion) && is_array($evaluacion)) {
            $puntajeUsado = $evaluacion['puntaje'] ?? $intento->puntaje;
            $totalUsado = $evaluacion['total_preguntas'] ?? $intento->total_preguntas;
            $detallesUsados = $evaluacion['detalles'] ?? null;
        } else {
            $puntajeUsado = $intento->puntaje;
            $totalUsado = $intento->total_preguntas;
            $detallesUsados = null;
        }

        $porcentaje = 0;
        if ($totalUsado > 0) {
            $porcentaje = round(($puntajeUsado / $totalUsado) * 100, 1);
        }
    ...
    <div class="result-header" style="background: linear-gradient(135deg, {{ $colorResultado }}, {{ $colorResultado }}dd);">
        ...
        <div class="score-card">
            ...
            <div class="score-details">{{ $puntajeUsado }} de {{ $totalUsado }} respuestas correctas</div>
    ...
    <div class="details-card">
        ...
        @php
            ...
            // Si la evaluación centralizada proporcionó detalles, úsala como fuente única
            if (isset($detallesUsados) && is_array($detallesUsados)) {
                $preguntasDetalles = $detallesUsados;
            } else {
                $preguntasDetalles = null; // fallback: generar a partir del contenido como antes
            }
        @endphp

        @if(!empty($preguntas))
            @if($preguntasDetalles !== null)
                {{-- Renderizar usando los detalles calculados por el servidor --}}
                @foreach($preguntasDetalles as $detalle)
                    @php
                        $index = $detalle['index'] ?? 0;
                        $pregunta = ['pregunta' => $detalle['pregunta'] ?? null, 'opciones' => $detalle['opciones'] ?? []];
                        $esCorrecta = $detalle['correcta'] ?? false;
                        $opcionAlumnoKey = $detalle['respuesta_alumno_key'] ?? null;
                        $opcionAlumnoText = $detalle['respuesta_alumno_text'] ?? ($detalle['respuesta_alumno_raw'] ?? null);
                        $opcionCorrectaKey = $detalle['respuesta_correcta_key'] ?? null;
                        $opcionCorrectaText = $detalle['respuesta_correcta_text'] ?? ($detalle['respuesta_correcta_raw'] ?? null);
                    @endphp
                            ...
                            <div>
                                ...
                                <ul class="options-list">
                                    @foreach($pregunta['opciones'] as $opcionKey => $opcionTexto)
                                        @php
                                            $esEstaLaRespuestaCorrecta = (strtoupper($opcionKey) === strtoupper((string)($opcionCorrectaKey ?? '')));
                                            $esEstaLaRespuestaDelAlumno = (strtoupper($opcionKey) === strtoupper((string)($opcionAlumnoKey ?? '')));
                                        @endphp
